...
function isLeapYear(date) {
  return date.getFullYear() !== 1900 && date.getFullYear() % 4 === 0;
}
...
function angleBetweenClockHands(date) {
  ...
  if (hour > 12) hour -= 12;
  let angleHour = hour * 30 + 0.5 * min;
  if (angleHour - 6 * min > 180) angleHour -= 180;

  const angle = Math.abs(angleHour - 6 * min);
